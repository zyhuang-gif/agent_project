...
3. handler 返回后只在真的成功（无 isError、无 error 字段、且拿到 message_id）
   时才写 log/sends.log；否则把 result 视为失败（必要时改写 isError=True），
   不写审计，防止认证/API 失败被误记为成功。文件类工具成功审计里额外附带
   file_key。
"""
import json
...
def _feishu_preview(args: dict, tool_name: str) -> str:
    """审计里用的 subject_or_preview——文件类给文件名，消息类给正文前 120 字。"""
...
class SendGuardInterceptor:
    ...
    async def _guard_gmail(
        ...
        record = {
            ...
            "subject_or_preview": (args.get("subject") or "")[:120],
    ...
    async def _guard_feishu(
        ...
        record = {
            ...
            "channel": "feishu",
            "recipient": recipient,
            "subject_or_preview": _feishu_preview(args, tool_name),
            "message_id": message_id,
        }
        if tool_name in FEISHU_FILE_TOOLS:
            file_key = _extract_file_key(payloads)
            if file_key:
